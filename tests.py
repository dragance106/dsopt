...
def smac_RF_coco(f_from=1, f_to=24, n_repeat=8, n_trials=400):
    for f_index in range(f_from, f_to+1):
        for repeat in range(n_repeat):
            ...
            fun = getattr(coco_smac, f'smac_coco_{f_index}')
...
class CocoFunction(OptimizationProblem):
    ...
    def eval(self, x):
        ...
        # FOR WHO KNOWS WHAT REASON, REFERENCING m HERE REFUSES TO WORK...
        # return fun([int(x[m['ia2a']]), int(x[m['ia2b']])],
        #            [int(x[m['ia4a']]), int(x[m['ia4b']])],
        #            [int(x[m['ia8a']]), int(x[m['ia8b']])],
        #            [int(x[m['ia16a']]), int(x[m['ia16b']])],
        #            [x[m['raa']], x[m['rab']]],
        #            int(x['ca']), int(x['cb']))

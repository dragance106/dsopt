        ...
        for key, value in arg_dict.items():
            if type(value) is tuple:
                if type(value[0]) is list or type(value[0]) is np.ndarray:
                    # value is ([array], 'c'), a categorical scalar
                    length = len(value[0])
                    idx = min(int(math.floor(p[index] * length)), length - 1)
                    arg_values[i][key] = value[0][idx]
                    index += 1
                else:
                    # value is (K, [array]) or (K, [array], 'c'), a numerical or a categorical array
                    length = len(value[1])
                    arg_values[i][key] = [
                        value[1][min(int(math.floor(p[index + j] * length)), length - 1)]
...
def hypercube_to_sample(hyperpoints, dim, arg_type, arg_array, arg_len):
    ...
    for i in range(dim):
        ...
        np.clip(indices, 0, arg_len[i] - 1, out=indices)
